...
void setup() {
  ...
  relayState1 = preferences.getInt("relayState1", LOW);
  relayState2 = preferences.getInt("relayState2", HIGH);
  ...
  if (esp_now_add_peer(&peerInfo) != ESP_OK){
    Serial.println("Failed to add ppreferences librarteer");
    return;
  }
  // Route for root / web page
  esp_sleep_enable_timer_wakeup(15 * 1000000); // wake up after 30 seconds
}
...
void loop() {
  ...
  if (doorState ==LOW){
    if (temperature > incomingReadings.thres3 && incomingReadings.checkbox3 == "true" && !triggerActive){
      Serial.println ("Turn the freezer unit on");
      triggerActive = true;
      digitalWrite (RELAY_PIN2, LOW);  //turn on
  ...
  if (doorState == HIGH) {
    ...
    // Save the state of the GPIO pins to the preferences memory
  preferences.putInt("relayState1", digitalRead(RELAY_PIN1));
  preferences.putInt("relayState2", digitalRead(RELAY_PIN2));
